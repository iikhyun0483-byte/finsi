...
// 현재가 조회
export async function getCurrentPrice(symbol: string): Promise<{ success: boolean; price?: number; error?: string }> {
  if (!KIS_APP_KEY || !KIS_APP_SECRET) {
    return { success: false, error: 'KIS 연동 예정입니다' }
  }
  ...
  try {
    ...
    const data = await res.json()
    return { success: true, price: Number(data.output?.stck_prpr ?? 0) }
  } catch (error) {
...
export async function placeBuyOrder(
  ...
  try {
    ...
    if (success) {
      return { success: true, orderNo: data.output?.KRX_FWDG_ORD_ORGNO ?? '' }
    } else {
      return { success: false, error: data.msg1 ?? '주문 실패' }
    }
  } catch (error) {
    return { success: false, error: (error as Error).message }
  }
