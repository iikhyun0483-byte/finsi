# PHASE 4 — 인생 라이프사이클 모델 (하드코딩 없는 버전)
# 탄생부터 사망까지 — 모든 값은 사용자 직접 입력

---

## 핵심 원칙

- DEFAULT_EVENTS 없음 — 이벤트 전부 사용자가 직접 추가
- 기본 나이, 기본 금액, 기본 수익률 하드코딩 없음
- 날짜 입력: "몇 살" 또는 "YYYY-MM-DD" 둘 다 지원
- 기간: "3년 후" "45세" "2041-01-01" 전부 지원

---

## STEP 1. lib/lifecycle-model.ts 생성
...
## STEP 2. app/api/lifecycle/route.ts 생성
...
## STEP 3. app/lifecycle/page.tsx 생성
...
export default function LifecyclePage() {
  ...
  async function simulate() {
    ...
    const required = [
      ...
      ['monthlyIncome', '월 소득'],
      ['monthlyExpense', '월 지출'],
      ['investmentReturn', '연 투자수익률'],
      ['retirementMonthlyExpense', '은퇴 후 월 지출'],
      ['pensionMonthly', '국민연금 월액 (없으면 0)'],
